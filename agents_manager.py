...
class AgentManager:
    ...
    def enable_file_search(self) -> None:
        ...
        file_streams = [open(os.path.join(os.path.dirname(__file__), p), "rb") for p in file_paths]
        self.client.beta.vector_stores.file_batches.upload_and_poll(vector_store_id=vector_store.id, files=file_streams)
    ...
    def create_agent(self, name: str, instructions: str, description: str = "") -> None:
        ...
        self.agent = Agent(name=name, instructions=instructions, model=self.model, tools=tools)
    ...
    async def run_agent(self, instructions: str = "") -> None:
        ...
        agent = self.agent if not instructions else self.agent.clone(instructions=instructions)
